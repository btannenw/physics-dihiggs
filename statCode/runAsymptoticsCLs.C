...
//check inputs
  ...
  cout << "INFO::Dataset: " << dataName << " was produced!" << endl;
  
  /*  // ORIGINAL
  //RooAbsPdf* pdf = mc->GetPdf();
  obs_nll = createNLL(data);//(RooNLLVar*)pdf->createNLL(*data);
  */
  RooAbsPdf* pdf = mc->GetPdf(); //BBT, Sept 1 2016
  obs_nll = (RooNLLVar*)pdf->createNLL(*data); //BBT, Sept 1 2016
  ...
  {
    std::cout<<"&&&& a"<<std::endl;
    //asimovData_0 = makeAsimovData(conditionalExpected, obs_nll, 1); // BBT, Aug 26 2016, generate asimov with mu=1
    //asimovData_0 = makeAsimovData(true, obs_nll, 0); // BBT, Aug 26 2016, make conditional explicit
    asimovData_0 = makeAsimovData(conditionalExpected, obs_nll, 0); // original
    
    //asimovData_0 = makeAsimovData2((conditionalExpected ? obs_nll : (RooNLLVar*)NULL), 0., 0.);
  }
  int asimov0_status=global_status;
  
  asimov_0_nll = createNLL(asimovData_0);//(RooNLLVar*)pdf->createNLL(*asimovData_0); // ORIGINAL
  //asimov_0_nll = (RooNLLVar*)pdf->createNLL(*asimovData_0); // BBT, Sept 1 2016
  ...
  double med_limit = doExp ? getLimit(asimov_0_nll, 0.2) : 1.0; //original
  ...
  if(doInj)
  {
    std::cout<<"&&&& d_inject [shouldn't pass]"<<std::endl;
    double mu_inj = 1.;
    if ( w->var("ATLAS_norm_muInjection") ) {
       mu_inj = w->var("ATLAS_norm_muInjection")->getVal();
    } else {
       mu_inj = w->var("SigXsecOverSM")->getVal(); // for the mass point at the inj
    }
...
double getLimit(RooNLLVar* nll, double initial_guess)
{
  ...
  global_status=0;
  cout<<"&&&& q: getSima, firstPOI->isConstant(): "<<firstPOI->isConstant()<<endl;
  if (nll == asimov_0_nll) 
  {
    std::cout<<"&&&& !!! 0"<<std::endl;
    setMu(0);
    firstPOI->setConstant(1);
    cout<<"&&&& FLAG, nll"<<endl;
    } // BBT, Sept 1 2016: comment out block to let mu float in fit. Hopefully this is okay
  cout<<"&&&& w: getSima, firstPOI->isConstant(): "<<firstPOI->isConstant()<<endl;
  double muhat;
  ...
  std::cout<<"&&&& e, muhat = "<<muhat<<std::endl;
  cout<<"&&&& r1: getSigma, firstPOI->isConstant(): "<<firstPOI->isConstant()<<endl;
  if (muhat < 0.1 || initial_guess != 0) setMu(initial_guess);
  cout<<"&&&& r2: getSigma, firstPOI->isConstant(): "<<firstPOI->isConstant()<<endl;
  double qmu,qmuA;
  double sigma_guess = getSigma(asimov_0_nll, firstPOI->getVal(), 0, qmu);
  double sigma_b = sigma_guess;
  cout<<"0 sigma_guess: "<<sigma_guess<<", muhat: "<<muhat<<std::endl;
  double mu_guess = findCrossing(sigma_guess, sigma_b, muhat);
  cout<<"1 mu_guess: "<<mu_guess<<std::endl;
  double pmu = calcPmu(qmu, sigma_b, mu_guess);
  cout<<"2 mu_guess: "<<mu_guess<<std::endl;
  double pb = calcPb(qmu, sigma_b, mu_guess);
  cout<<"3 mu_guess: "<<mu_guess<<std::endl;
  double CLs = calcCLs(qmu, sigma_b, mu_guess);
  cout<<"4 mu_guess: "<<mu_guess<<std::endl;
  double qmu95 = getQmu95(sigma_b, mu_guess);
  cout<<"5 mu_guess: "<<mu_guess<<std::endl;
  std::cout<<"&&&& t"<<std::endl;
  setMu(mu_guess);
  ...
  cout<<"(0)()()() mu_pre: "<<mu_pre<<", mu_guess: "<<mu_guess<<", precision: "<<precision<<", direction: "<<direction<<", fabs(mu_pre-mu_guess) = "<<fabs(mu_pre-mu_guess)<<", precision*mu_guess*direction = "<<precision*mu_guess*direction<<endl;

  while (fabs(mu_pre-mu_guess) > precision*mu_guess*direction)
  {
    cout<<"(1)()()() mu_pre: "<<mu_pre<<", mu_guess: "<<mu_guess<<", precision: "<<precision<<", direction: "<<direction<<", fabs(mu_pre-mu_guess) = "<<fabs(mu_pre-mu_guess)<<", precision*mu_guess*direction = "<<precision*mu_guess*direction<<endl;
    cout << "----------------------" << endl;
...
double getSigma(RooNLLVar* nll, double mu, double muhat, double& qmu)
{
  cout<<"^^^ a: getSima, firstPOI->isConstant(): "<<firstPOI->isConstant()<<endl;
  qmu = getQmu(nll, mu);
  std::cout<<"^^^ qmu = "<<qmu<<", nll = "<<nll->GetName()<<", mu = "<<mu<<", muhat = "<<muhat<<", direction = "<<direction<<std::endl;
...
double getQmu(RooNLLVar* nll, double mu)
{
  ...
  cout << "qmu = 2 * (" << nll_val << " - " << nll_muhat << "), firstPOI->isConstant(): " <<firstPOI->isConstant()<< endl;
...
void saveSnapshot(RooNLLVar* nll, double mu)
{
  ...
  w->saveSnapshot(snapshotName.str().c_str(), (mc->GetNuisanceParameters() ? *mc->GetNuisanceParameters() : RooArgSet()));
}
...
double getNLL(RooNLLVar* nll)
{
  ...
  cout<<"¬¬¬¬ snapshotName: "<<snapshotName<<endl;
  minimize(nll);
...
int minimize(RooAbsReal* fcn)
{
  ...
  int strat = ROOT::Math::MinimizerOptions::DefaultStrategy(); // original
  //int strat = 0; // BBT Aug 31, 2016
...
RooDataSet* makeAsimovData(bool doConditional, RooNLLVar* conditioning_nll, double mu_val, string* mu_str, string* mu_prof_str, double mu_val_profile, bool doFit, double mu_injection)
{
  ...
  cout << "Creating asimov data at mu = " << mu_val << ", profiling at mu = " << mu_val_profile << ", doConditional = "<<doConditional<< endl;
...
////////////////////
  ...
  mu->setVal(mu_val);
  cout << "Constatnt POI ";
  if(mu->isConstant()) { cout << "YES" << endl; } else { cout << "NO" << endl; }
  cout << "Value of POI  " << mu->getVal() << endl;
...
//save the snapshots of nominal parameters, but only if they're not already saved
  ...
  mu->setConstant(1);
  //int status = 0;
  cout<<"@@@ doConditional: "<<doConditional<<", doFit: "<<doFit<<endl;
